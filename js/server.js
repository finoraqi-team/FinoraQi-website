...
/**
 * Propósito: Server Node opcional para:
 * - Proxy de API em ambientes sem CORS configurado
 * - Server-side rendering (SSR) se necessário
 * - Fallback para WebSocket em browsers antigos
 * 
 * Uso: Apenas se o frontend não puder chamar a API Python diretamente
 */
...
// Proxy para API Python (contorna CORS em desenvolvimento)
app.use('/api', createProxyMiddleware({
    target: API_URL,
    changeOrigin: true,
    pathRewrite: { '^/api': '/api' },
    onProxyReq: (proxyReq, req) => {
        // Forward headers de auth se presentes
...
// WebSocket proxy (opcional - se necessário)
...
wss.on('connection', (ws, req) => {
    // Proxy de WebSocket para o backend Python
    const targetWs = new WebSocket(`${API_URL.replace('http', 'ws')}${req.url}`);
...
app.get('/health', async (req, res) => {
    try {
        const backendHealth = await fetch(`${API_URL}/health`);
        const data = await backendHealth.json();
        res.json({ proxy: 'ok', backend: data });
    } catch {
        res.status(503).json({ proxy: 'ok', backend: 'unreachable' });
    }
});

server.listen(PORT, () => {
    console.log(`Proxy server rodando em http://localhost:${PORT}`);
    console.log(`🔗 API Python em ${API_URL}`);
});